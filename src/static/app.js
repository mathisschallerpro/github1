      ...
      Object.entries(activities).forEach(([name, details]) => {
        ...
        const spotsLeft = details.max_participants - details.participants.length;
          ...
          const participantsHtml = (details.participants && details.participants.length)
            ? `<div class="participants"><strong>Participants:</strong><ul class="participants-list">${details.participants.map(p => `<li class="participant-item"><span class="participant-email">${p}</span><button class="participant-remove" data-activity="${encodeURIComponent(name)}" data-email="${encodeURIComponent(p)}" title="Unregister participant">✕</button></li>`).join('')}</ul></div>`
            : `<div class="participants"><strong>Participants:</strong><p class="info">No participants yet</p></div>`;
